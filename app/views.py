...
def view_user_id(request,pk):
    ...
    return render(request,'app/user/user_profile.html',{'user':user},)
...
def contact(request):
    if request.method == 'POST':
        ...
        send_mail('Contact Form Message From My Ride', send_message, 'abuuanas30a@hotmail.com', ['buuanas30a@hotmail.com'])
        if request.user.is_authenticated():
...
def vehicle_share(request, user_id, vehicle_id):
    ...
    if request.method == "POST":
        if form.is_valid():
            ...
            form.fields['cost'] = fm.IntegerField(widget=forms.TextInput(attrs={'type':'number', 'min':"10", 'required':True,'placeholder':'In kes ','class':'form-control'}))
